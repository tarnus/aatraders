...
class ADODB_Compress_Bzip2 {
	...
	function write($data, $key) {
		...
		if (!is_null($this->_block_size)) {
			if (!is_null($this->_work_level)) {
				return base64_encode(bzcompress($data, $this->_block_size, $this->_work_level));
			} else {
				return base64_encode(bzcompress($data, $this->_block_size));
			}
		}
	...
	function read($data, $key) {
		return $data ? base64_decode(bzdecompress($data)) : $data;
	}
