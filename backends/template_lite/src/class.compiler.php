	...
	function _parse_function($function, $modifiers, $arguments)
	{
		switch ($function) {
			...
			case 'foreach':
				...
				$foreach_props = isset($_args['name']) ? "\$this->_foreach['" . $_args['name'] . "']" : "\$this->_foreach['foreach" . count($this->_foreachelse_stack) . "']";
				isset($_args['key']) ? $_args['key'] = "\$this->_vars['".$this->_dequote($_args['key'])."'] => " : $_args['key'] = '';
				$_result = "<?php {$foreach_props} = array('total' => count((array)" . $_args['from'] . "), 'iteration' => 0);\n";
				$_result .= "if (count((array)" . $_args['from'] . ")): foreach ((array)" . $_args['from'] . " as " . $_args['key'] . "\$this->_vars['" . $_args['value'] . "']): {$foreach_props}['iteration']++;\n ?>";
				return $_result;
				break;
			case 'foreachelse':
			...
			if ($variable[0]{0} == '[' || $variable[0]{0} == '.')
			{
				...
				switch(strtoupper(str_replace($find, "", $variable[0])))
				{
					case 'FOREACH':
						...
						$_var = substr(array_shift($variable), 1);
						$_propname = substr($variable[0], 1);
						switch (strtoupper($_propname))
						{
							case 'INDEX':
								array_shift($variable);
								$_result = "(\$this->_foreach['$_var']['iteration']-1)";
								break;
							case 'ITERATION':
								array_shift($variable);
								$_result = "(\$this->_foreach['$_var']['iteration'])";
								break;
							case 'FIRST':
								array_shift($variable);
								$_result = "(\$this->_foreach['$_var']['iteration'] <= 1)";
							...
							case 'LAST':
								array_shift($variable);
								$_result = "(\$this->_foreach['$_var']['iteration'] == \$this->_foreach['$_var']['total'])";
								break;
							case 'SHOW':
								array_shift($variable);
								$_result = "(\$this->_foreach['$_var']['total'] > 0)";
								break;
							default:
								$_result = "\$this->_foreach['$_var']['total']";
						}
					break;
					case 'GET':
						$_result = "\$_GET";
