...
class Template_Lite {
	...
	var $compile_id		= null;		// Set this if you want different sets of compiled files for the same templates. 
	...
	function fetch($file, $cache_id = null, $compile_id = null, $display = false)
	{
		...
		$name = $this->_get_compile_name($file);
...
//		$this->_error_level = error_reporting(E_STRICT);
...
//		$name = ($this->encode_file_name) ? md5((($this->_resource_type == 1) ? $this->template_dir.$file : $this->_resource_type . "_" . $file)).'.php' : str_replace(".", "_", str_replace("/", "_", $this->_resource_type . "_" . $file)).'.php';
	...
	function _get_compile_name($file)
	{
		$name = (($this->encode_file_name) ? md5((($this->_resource_type == 1) ? $this->template_dir.$file : $this->_resource_type . "_" . $file)).'.php' : str_replace(".", "_", str_replace("/", "_", $this->_resource_type . "_" . sprintf("%u", crc32($this->template_dir)) . "_" . $file)).'.php');
		return $name;
	}
