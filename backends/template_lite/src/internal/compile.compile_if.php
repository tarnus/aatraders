	...
	for ($i = 0, $for_max = $count_args; $i < $for_max; $i++)
	{
		...
		switch (strtolower($_arg))
		{
			...
			case 'is':
				if ($_args[$i-1] == ')')
				{
					$_is_arg_start = array_pop($_is_arg_stack);
				}
				else
				{
					$_is_arg_count = count($_args);
					$is_arg = implode(' ', array_slice($_args, $_is_arg_start, $i - $_is_arg_start));
					$_arg_tokens = $object->_parse_is_expr($is_arg, array_slice($_args, $i+1));
					array_splice($_args, $_is_arg_start, count($_args), $_arg_tokens);
