...
get_post_ifset("startsector, turns");
...
if($enable_spiral_galaxy != 1){
	$query2 = "SELECT sector_name, spiral_arm, sector_id, port_type, zone_id, x, y, z,SQRT((($sectorinfo[x]-x) * ($sectorinfo[x]-x))+(($sectorinfo[y]-y) * ($sectorinfo[y]-y))+(($sectorinfo[z]-z) * ($sectorinfo[z]-z))) as sector_distance FROM {$db_prefix}universe where SQRT((($sectorinfo[x]-x) * ($sectorinfo[x]-x))+(($sectorinfo[y]-y) * ($sectorinfo[y]-y))+(($sectorinfo[z]-z) * ($sectorinfo[z]-z))) < $distance and sector_id!=$shipinfo[sector_id] and sg_sector = 0  ORDER BY sector_distance ASC,sector_id DESC";
}
else
{
	$query2 = "SELECT sector_name, spiral_arm, sector_id, port_type, zone_id, x, y, z,SQRT(POW(($sectorinfo[x]-x),2)+POW(($sectorinfo[y]-y),2)+POW(($sectorinfo[z]-z),2)) as sector_distance FROM {$db_prefix}universe where SQRT(POW(($sectorinfo[x]-x),2)+POW(($sectorinfo[y]-y),2)+POW(($sectorinfo[z]-z),2)) < $distance and sector_id!=$shipinfo[sector_id] and sg_sector = 0  ORDER BY sector_distance ASC,sector_id DESC";
}
...
while (!$result->EOF)
{
	...
	$port= "unknown";
//	$alt = "$row[sector_name] - $l_unknown - " . round(max(1, (calc_dist($shipinfo['sector_id'],$row['sector_id'])/ mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
	$alt = "$row[sector_name] - $l_unknown - " . round(max(1, ($row['sector_distance'] / mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
	$altsector[$mapsectorcount] = $row['sector_name'];
	$altport[$mapsectorcount] = $l_unknown;
//	$altturns[$mapsectorcount] = round(max(1, (calc_dist($shipinfo['sector_id'],$row['sector_id'])/ mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
	$altturns[$mapsectorcount] = round(max(1, ($row['sector_distance'] / mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
	$zonecolor = "#000000";
	$link_list[$mapsectorcount] = $l_none;
	...
	if ($sectorzone[$tempsector] > 0 )
	{
		...
		$port = $row['port_type'];
//		$alt  = "$row[sector_name] $distance- $text[$port] - ".$zonename[$temp] . " - " . round(max(1, (calc_dist($shipinfo['sector_id'],$row['sector_id'])/ mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
		$alt  = "$row[sector_name] $distance- $text[$port] - ".$zonename[$temp] . " - " . round(max(1, ($row['sector_distance'] / mypw($level_factor, $shipinfo['engines'])))) . " realspace turns";
		$altsector[$mapsectorcount]  = $row['sector_name'];
...
$template_object->assign("altturns", $altturns);
...
$template_object->display($templatename."galaxy_local.tpl");
include ("footer.php");
