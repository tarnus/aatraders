...
{
	if(time() > (@filemtime($gameroot . "support/rankingupdate.ini") + 1800) || !file_exists($gameroot . "support/rankingupdate.ini"))
	{
		$file = fopen($gameroot . "support/rankingupdate.ini","w") or die ("Failed opening file: enable write permissions for '" . $gameroot . "support/rankingupdate.ini'");
		fwrite($file, time()); 
		fclose($file);
...
elseif ($sort == "team")
{
	$by = "{$db_prefix}teams.team_name ASC, character_name ASC";
}
...
$res = $db->Execute("SELECT {$db_prefix}players.kill_efficiency, {$db_prefix}players.experience, {$db_prefix}players.email, {$db_prefix}players.score, {$db_prefix}players.attackscore, {$db_prefix}players.player_id, " .
                        ...
                        "{$db_prefix}players.turns_used, UNIX_TIMESTAMP({$db_prefix}players.last_login) as last_login, " .
                        "UNIX_TIMESTAMP({$db_prefix}players.last_login) as online, {$db_prefix}players.rating, " .
