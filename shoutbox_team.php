...
$res1 = $db->SelectLimit("SELECT count(sb_alli) as total FROM {$db_prefix}shoutbox WHERE sb_alli = " . $playerinfo['team'], 1);
...
$query = "SELECT * FROM {$db_prefix}shoutbox WHERE sb_alli = $playerinfo[team] ORDER BY sb_date desc ";
...
$template_object->assign("l_shout_teamtitle", $l_shout_teamtitle);
$template_object->assign("l_global_mmenu", $l_global_mmenu);
$template_object->display($templatename."shoutbox_team.tpl");
