...
<table width="80%" border="1" cellspacing="0" cellpadding="4" align="center">
	{if $totaldigs}
		<TR BGCOLOR="#000000"><td><font color=white><b>{$l_dig_legend}</b></font></td><td><font color=white><b>{$l_dig_max}</b></font></td><TD><font color=white><B>{$l_dig_description}</B></font></TD></TR>
	...
	<br>
		...
		{if $totaldigsbyplanet}
			...
			{/php}
			<TR BGCOLOR="#000000">
			<TD colspan=5 align=center><INPUT TYPE="hidden" name="digcount" value="{$digcount}">
			<INPUT TYPE="submit" value="{$l_dig_changebutton}"></td></tr>
			</FORM>
			</TABLE><BR><BR>
		{else}
			<B>{$l_dig_no2}</B><BR><BR>
