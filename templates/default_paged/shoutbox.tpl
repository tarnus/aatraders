...
<TR BGCOLOR='#585980'>
<TH COLSPAN=2 NOWRAP><FONT COLOR=yellow>{$l_shout_title2}</FONT></TH>
</TR>
  <tr>
...
{if $totalpages > 1}
	...
	<TR>
		<td align="left" width="33%">
			{if $currentpage != 1}
				<a href="shoutbox.php?page={$previouspage}">{$l_shout_prev}</a>
			{else}
				&nbsp;
			{/if}
		</td>
	...
	&nbsp;<input type="submit" value="{$l_submit}">
		...
		<td align="right" width="33%">
			{if $currentpage != $totalpages}
				<a href="shoutbox.php?page={$nextpage}">{$l_shout_next}</a>
			{else}
				&nbsp;
			{/if}
		</td>
...
<INPUT TYPE="hidden" NAME="SBPB" value="1">
