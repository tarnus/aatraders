...
{
	return (mypw({/literal}{$upgrade_factor}{literal}, desiredvalue) - mypw({/literal}{$upgrade_factor}{literal}, currentvalue)) * {/literal}{$upgrade_cost}{literal} * {/literal}{$alliancefactor}{literal} * ({/literal}{$repair_modifier}{literal} / 100);
}
...
<FORM name="spacedockview" ACTION=port_purchase.php METHOD=POST>
<table width="600" border="1" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td class="header"><h3>{$l_port_welcome}</h3></td>
  </tr>
  <tr>
    <td class="leftColumn"><div align="center"><img src="images/ports/port_spacedock.gif" alt="spacedock" /><br><b>{$l_spacedock}</b></div></td>
</td>
  </tr>
  <tr>
...
<table width="600" border="1" align="center" cellpadding="5" cellspacing="0">
  ...
  <tr>
    <td class="leftBuyColumn" nowrap>{$l_ship_levels}</td>
    <td class="rightBuyColumn">{$l_cost}</td>
    <td class="centerColumn" nowrap>{$l_current_level}</td>
    <td class="rightBuyColumn">{$l_ports_needrepair}</td>
  </tr>
  ...
  <tr><td colspan="4" align="center" class="portfooter">{$l_totalcost} <input type="text" class="inputcss" style="text-align:right" name="total_cost2" size="32" value="0" {$onfocus} {$onblur} {$onchange} {$onclick}><br><br>
	<input type="hidden" name="total_cost" value="0">
  	<input name="perform" type="submit" class="groovybutton" value="{$l_ports_needrepair}" {$onclick} onMouseOver="goLite(this.form.name,this.name)" onMouseOut="goDim(this.form.name,this.name)"/>
